...
Modalbox.Methods={
		...
		_init:function(options){
			...
			if(!this.options.overlay)
				$(this.MBoverlay).setStyle({display:'block'});
			...
			if(this.initialized){
				...
				if(this.options.transitions)
					Effect.SlideUp(this.MBwindow,
							{duration:this.options.slideUpDuration,transition:Effect.Transitions.sinoidal,afterFinish:this._deinit.bind(this)});
				else{
					$(this.MBwindow).hide();
					this._deinit();
				}
		...
		autoHide:function(seconds,options){
			if(this._pe)
				this._pe.stop();
			this._pe=new PeriodicalExecuter(function(pe){pe.stop();this.hide(options);}.bind(this),seconds);
		},
		...
		resize:function(byWidth,byHeight,options){
			var oHeight=$(this.MBoverlay).getHeight();
		...
		this.MBwindow.setStyle({height:"auto",top:((oHeight-newHeight)/2)+"px"});
	   },
	   resizeToContent:function(options){
		...
		_update:function(){
			...
			$(this.MBcontent).setStyle({height:'auto'});this.loadContent();
		},
	  ...
	  _insertContent:function(content,callback){
		  ...
		  if(typeof content=='string'){
			  ...
			  if(content.id)
				  content.id="MB_"+content.id;$(content).select('*[id]').each(function(el){el.id="MB_"+el.id;});
			  ...
			  if(Prototype.Browser.IE)
	  		  ...
	  		  if(this.options.height==this._options.height){
	  			 ...
	  			 setTimeout(function(){this._putContent(callback);}.bind(this),1);
	  		 }
	 },
	...
	_setFocus:function(){
		if(this.focusableElements.length>0&&this.options.autoFocusing==true){
			var firstEl=this.focusableElements.find(function(el){return el.tabIndex==1;})||this.focusableElements.first();
	...
	_setPosition:function(){
		...
		$(this.MBwindow).setStyle({top:(($(this.MBoverlay).getHeight()-$(this.MBwindow).getHeight())/2)+"px"});  //add default position top  by xhs 20121031
	},

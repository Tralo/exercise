...
class Magmi_ProductImportEngine extends Magmi_Engine
{
	...
	public function onEngineException($e)
	{
        ...
        /**
         * 自动导入当前自己所有的simple products
         * @param type $simple_products
         * @return array 所有simple的SKU数组
         */
        public function walkAllSimpleProducts($simple_products)
        {
            $skus = array();
            foreach($simple_products as $item){
                
                if(!($this->getProductIds($item['sku']))){
                    $engin = clone $this;
                    $engin->importItem($item);
                }
                $skus[] = $item['sku'];
            }
            return $skus;
         ...
         * @return 当前处理的行数
         */
        public function getCurrentImportRow()
        {
            return $this->_current_row;
        }
}